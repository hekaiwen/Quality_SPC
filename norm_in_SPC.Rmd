...
```{r dnorm_sigma,echo=FALSE,warning=FALSE,fig.height=3,fig.width=9}
  ...
  color_area <- c("#BFBFCF","#BFCFBF","#BFEFEF","#BFFFFF","#BFBFBF","#C00000")
 ...
 p17 <- ggplot(df17,aes(x=x,y=d)) + 
   ...
   geom_area(fill="#A2F4BA",alpha = .25) + 
   scale_x_continuous(limits = x_limits,breaks = x_break,labels = x_labls)+
   geom_vline(aes(xintercept=0))+
    geom_area(data =subset(df17,x< -3*sd*0.7),aes(x=x,y=d),fill=color_area[1]) +
    geom_area(data =subset(df17,x> 3*sd*0.7),aes(x=x,y=d),fill=color_area[1])+
   theme_hkw()          
...
```{r histgram,echo=FALSE,warning=FALSE,fig.height=4,fig.width=9}
set.seed(2026)

  data1 <- round(rnorm(1000,mean,sd),3)
  df1 <- data.frame(x = data1,d= dnorm(data1,mean,sd))
  kable(head(df1,10),caption = "10个数据")
  
  p1 <- ggplot(df1,aes(x=x,y=d)) + 
    geom_histogram(aes(y=..density..),color ="#17C2B2",alpha = .25,fill="#A2F4BA",binwidth = 0.01) + 
    labs(title= "100个数据的频数分布")  +
    scale_x_continuous(limits = x_limits,breaks = x_break)+
    theme_hkw()

data2 <- round(rnorm(1000,mean,sd),3)
  df2 <- data.frame(x = data2,d= dnorm(data2,mean,sd))
  p2 <- ggplot(df2,aes(x=x,y=d)) + 
    geom_histogram(aes(y=..density..),color ="#17C2B2",alpha = .25,fill="#A2F4BA",binwidth =0.005) +
    labs(title="1000个数据的频数分布")  +
    scale_x_continuous(limits = x_limits,breaks = x_break)+
  theme_hkw()
  
  data3 <- round(rnorm(10000,mean,sd),3)
  df3 <- data.frame(x = data3,d= dnorm(data3,mean,sd))
  p3 <- ggplot(df3,aes(x=x,y=d)) + 
    geom_histogram(aes(y=..density..),color ="#17C2B2",alpha = .25,fill="#A2F4BA",binwidth = 0.002) +
  ...
  p4 <- ggplot(df4,aes(x=x,y=d)) + 
    geom_histogram(aes(y=..density..),color ="#17C2B2",alpha = .25,fill="#A2F4BA",binwidth = 0.001)  +
    labs(title="100000个正态分布数据的频数分布图") +
    scale_x_continuous(limits = x_limits,breaks = x_break)+
    theme_hkw()
```

```{r histgram_grid,echo=FALSE,warning=FALSE,fig.width=9,fig.height=12}
...
```{r hist_line,echo=FALSE,warning=FALSE,fig.height=3,fig.width=9}

  x_limits <- c(-7*0.7*sd,7*0.7*sd)
  x_break <- c(-6*0.7*sd,-5*0.7*sd,-4*0.7*sd,-3*0.7*sd,-2*0.7*sd,-0.7*sd,mean,0.7*sd,2*0.7*sd,3*0.7*sd,4*0.7*sd,5*0.7*sd,6*0.7*sd)
  x_labls<- expression(-6*sigma,-5*sigma,-4*sigma,-3*sigma,-2*sigma,-1*sigma,0,1*sigma,2*sigma,3*sigma,4*sigma,5*sigma,6*sigma)
p5 <- p4 + geom_line(color = "#186779") 
 p5
 
 df6 <- df4 %>% distinct()
 p6 <- ggplot(df6,aes(x=x,y=d)) + 
    labs(title="正态分布6sigma区间") + geom_line(color = "#186779") +    
   geom_area(fill="#A2F4BA",alpha = .25) + 
   scale_x_continuous(limits = x_limits,breaks = x_break,labels = x_labls)+
   theme_hkw()
 p6
 
 p7 <- p6 + geom_vline(aes(xintercept=0))
 p7
```
...
```{r hist_area,echo=FALSE,warning=FALSE,fig.height=3,fig.width=9}



  p8 <- p7 +
    geom_area(data =subset(df6,x< -sd*0.7),aes(x=x,y=d),fill=color_area[1]) +
    geom_area(data =subset(df6,x>sd*0.7),aes(x=x,y=d),fill=color_area[1])
    
  p8
  
  p9 <- p8 +
    geom_area(data =subset(df6,x< -2*sd*0.7),aes(x=x,y=d),fill=color_area[2]) +
    geom_area(data =subset(df6,x> 2*sd*0.7),aes(x=x,y=d),fill=color_area[2])
  p9
  p10 <- p9 +
    geom_area(data =subset(df6,x< -3*sd*0.7),aes(x=x,y=d),fill=color_area[3]) +
    geom_area(data =subset(df6,x> 3*sd*0.7),aes(x=x,y=d),fill=color_area[3])
  p10
  
  p11<- p10 +
    geom_area(data =subset(df6,x< -4*sd*0.7),aes(x=x,y=d),fill=color_area[5]) +
    geom_area(data =subset(df6,x> 4*sd*0.7),aes(x=x,y=d),fill=color_area[5])
  p11

  p12 <- p11 + 
    geom_area(data =subset(df6,x< -5*sd*0.7),aes(x=x,y=d),fill=color_area[6]) +
    geom_area(data =subset(df6,x> 5*sd*0.7),aes(x=x,y=d),fill=color_area[6])
...
```{r hist_6sigma,echo=FALSE,warning=FALSE,fig.height=3,fig.width=9}
 ...
 p17 <- ggplot(df17,aes(x=x,y=d)) + 
   ...
   scale_x_continuous(limits = x_limits,breaks = x_break,labels = x_labls)+
   geom_vline(aes(xintercept=0))+
   geom_area(data =subset(df17,x< -sd*0.7),aes(x=x,y=d),fill=color_area[1]) +
    geom_area(data =subset(df17,x>sd*0.7),aes(x=x,y=d),fill=color_area[1])+
    geom_area(data =subset(df17,x< -2*sd*0.7),aes(x=x,y=d),fill=color_area[2]) +
    geom_area(data =subset(df17,x> 2*sd*0.7),aes(x=x,y=d),fill=color_area[2])+
    geom_area(data =subset(df17,x< -3*sd*0.7),aes(x=x,y=d),fill=color_area[3]) +
    geom_area(data =subset(df17,x> 3*sd*0.7),aes(x=x,y=d),fill=color_area[3])+
    geom_area(data =subset(df17,x< -4*sd*0.7),aes(x=x,y=d),fill=color_area[5]) +
    geom_area(data =subset(df17,x> 4*sd*0.7),aes(x=x,y=d),fill=color_area[5]) +
    geom_area(data =subset(df17,x< -5*sd*0.7),aes(x=x,y=d),fill=color_area[6]) +
    geom_area(data =subset(df17,x> 5*sd*0.7),aes(x=x,y=d),fill=color_area[6])+ 
   geom_vline(aes(xintercept=mean + x_tol))+
   geom_vline(aes(xintercept=mean - x_tol))+
   theme_hkw()          
...
```{r}
  ...
  mean <- 0
  sd <- 0.007
  x_limits <- c(-7*sd/2,7*sd/2)
  x_break <- c(-6*sd/2,-5*sd/2,-4*sd/2,-3*sd/2,-2*sd/2,-sd/2,mean,sd/2,2*sd/2,3*sd/2,4*sd/2,5*sd/2,6*sd/2)
  x_tol<- 0.02    # tol 公差，也即设计标准。
  
  data14 <- round(rnorm(100000,mean,sd),4)  
  df14 <- data.frame(x = data14,d= dnorm(data14,mean,sd))    %>% distinct()
