...
sd <- 0.8
  data18 <- round(rnorm(100000,mean,sd),3)

  df18 <- data.frame(x = data18,d= dnorm(data18,mean,sd)) %>% distinct()
  df18$g <-"df18"
  
  p18 <- ggplot(df18,aes(x=x,y=d)) + 
   labs(title=expression(paste("正态分布6",sigma,"区间"))) + geom_line(color = color_area[1]) +  
   geom_area(fill=color_area[1]) + 
   scale_x_continuous(limits = x_limits,breaks = x_break,labels = x_labls)+
  scale_y_continuous(limits = c(0,0.8),breaks = c(0,0.2,0.4,0.6,0.8))+
   geom_vline(aes(xintercept=0))+
   geom_area(data =subset(df18,x< -sd*0.8),aes(x=x,y=d),fill=color_area[2]) +
    geom_area(data =subset(df18,x>sd*0.8),aes(x=x,y=d),fill=color_area[2])+
    geom_area(data =subset(df18,x< -2*sd*0.8),aes(x=x,y=d),fill=color_area[3]) +
    geom_area(data =subset(df18,x> 2*sd*0.8),aes(x=x,y=d),fill=color_area[3])+
    geom_area(data =subset(df18,x< -3*sd*0.8),aes(x=x,y=d),fill=color_area[4]) +
    geom_area(data =subset(df18,x> 3*sd*0.8),aes(x=x,y=d),fill=color_area[4])+
    geom_area(data =subset(df18,x< -4*sd*0.8),aes(x=x,y=d),fill=color_area[5]) +
    geom_area(data =subset(df18,x> 4*sd*0.8),aes(x=x,y=d),fill=color_area[5]) +
    geom_area(data =subset(df18,x< -5*sd*0.8),aes(x=x,y=d),fill=color_area[6]) +
    geom_area(data =subset(df18,x> 5*sd*0.8),aes(x=x,y=d),fill=color_area[6])+ 
   geom_vline(aes(xintercept=mean + x_tol))+
   geom_vline(aes(xintercept=mean - x_tol))+
   theme_hkw()  
  ...
  sd <- 0.6
  data19 <- round(rnorm(100000,mean,sd),3)

  df19 <- data.frame(x = data19,d= dnorm(data19,mean,sd)) %>% distinct()
  df19$g <- "df19"
  
  p19 <- ggplot(df19,aes(x=x,y=d)) + 
   labs(title=expression(paste("正态分布6",sigma,"区间"))) + geom_line(color = color_area[1]) +  
   geom_area(fill=color_area[1]) + 
    scale_x_continuous(limits = x_limits,breaks = x_break,labels = x_labls)+
   scale_y_continuous(limits = c(0,0.8),breaks = c(0,0.2,0.4,0.6,0.8))+
   ...
   geom_area(data =subset(df19,x< -sd*0.8),aes(x=x,y=d),fill=color_area[2]) +
    geom_area(data =subset(df19,x>sd*0.8),aes(x=x,y=d),fill=color_area[2])+
    geom_area(data =subset(df19,x< -2*sd*0.8),aes(x=x,y=d),fill=color_area[3]) +
    geom_area(data =subset(df19,x> 2*sd*0.8),aes(x=x,y=d),fill=color_area[3])+
    geom_area(data =subset(df19,x< -3*sd*0.8),aes(x=x,y=d),fill=color_area[4]) +
    geom_area(data =subset(df19,x> 3*sd*0.8),aes(x=x,y=d),fill=color_area[4])+
    geom_area(data =subset(df19,x< -4*sd*0.8),aes(x=x,y=d),fill=color_area[5]) +
    geom_area(data =subset(df19,x> 4*sd*0.8),aes(x=x,y=d),fill=color_area[5]) +
    geom_area(data =subset(df19,x< -5*sd*0.8),aes(x=x,y=d),fill=color_area[6]) +
    geom_area(data =subset(df19,x> 5*sd*0.8),aes(x=x,y=d),fill=color_area[6])+ 
   geom_vline(aes(xintercept=mean + x_tol))+
   geom_vline(aes(xintercept=mean - x_tol))+
   theme_hkw() 
...
```{r dnorm_3,fig.height=3,fig.width=9}
df20 <- bind_rows(df17,df18,df19)
  
  p20 <- ggplot(df20,aes(x=x,y=d,color = g)) + 
